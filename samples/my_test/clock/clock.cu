...
// in device memory.
__global__ static void timedReduction(const float *input, float *output, clock_t *timer, int * smid)
{
...
int main(int argc, char **argv)
{
    ...
    checkCudaErrors(cudaMalloc((void **)&dsm, sizeof(int) * NUM_BLOCKS));
    checkCudaErrors(cudaMemcpy(dinput, input, sizeof(float) * NUM_THREADS * 2, cudaMemcpyHostToDevice));

    timedReduction<<<NUM_BLOCKS, NUM_THREADS, sizeof(float) * 2 *NUM_THREADS>>>(dinput, doutput, dtimer,dsm);
    checkCudaErrors(cudaMemcpy(timer, dtimer, sizeof(clock_t) * NUM_BLOCKS * 2, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(smid, dsm, sizeof(int) * NUM_BLOCKS, cudaMemcpyDeviceToHost));

    checkCudaErrors(cudaFree(dinput));
    checkCudaErrors(cudaFree(doutput));
    checkCudaErrors(cudaFree(dtimer));
    checkCudaErrors(cudaFree(dsm));
    long double avgElapsedClocks = 0;
    
    for (int i = 0; i < NUM_BLOCKS; i++)
    {
        avgElapsedClocks += (long double) (timer[i + NUM_BLOCKS] - timer[i]);
        printf("CTA allocation, cta:%d, sm_id;%d", i,smid[i]);    
       	}
