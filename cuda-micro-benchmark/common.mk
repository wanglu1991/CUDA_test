...
SM_VERSIONS := sm_20 sm_30 sm_35 sm_37 sm_50 sm_52 sm_60
...
BUILDSCRIPT := $(ROOTDIR)/buildscript
...
$(OBJDIR)/%.ptx : $(SRCDIR)/%.cu
	$(VERBOSE)$(NVCC) --ptx $(NVCCFLAGS) $< -o $@

$(OBJDIR)/%.cu_sm_13_o : $(SRCDIR)/%.cu $(CU_DEPS)
	$(VERBOSE)$(NVCC) -o $@ -c $< -keep $(NVCCFLAGS) -arch sm_13
	$(VERBOSE)sh $(BUILDSCRIPT)/build_cubin $<

$(OBJDIR)/%.cu_sm_12_o : $(SRCDIR)/%.cu $(CU_DEPS)
	$(VERBOSE)$(NVCC) -o $@ -c $< -keep $(NVCCFLAGS) -arch sm_12
	$(VERBOSE)sh $(BUILDSCRIPT)/build_cubin $<
